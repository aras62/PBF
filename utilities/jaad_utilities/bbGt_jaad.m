function varargout = bbGt_jaad( action, varargin )
...
%
% See also bbGt
o=struct('lbl','','fname','','bb',[0 0 0 0],'occ',0,'bbv',[0 0 0 0],'ign',0,'ang',0);
...
df={'format',0,'ellipse',1,'squarify',[],'lbls',[],'ilbls',[],'hRng',[],...
    'wRng',[],'aRng',[],'arRng',[],'oRng',[],'xRng',[],'yRng',[],'vRng',[], 'ignOcc',0};
[format,ellipse,sqr,lbls,ilbls,hRng,wRng,aRng,arRng,oRng,xRng,yRng,vRng,ignOcc]...
...
if(~isempty(lbls) || ~isempty(ilbls)), K=true(n,1);
    for i=1:n
        K(i)=any(strcmp(objs(i).lbl,[lbls ilbls]));
    end
    objs=objs(K);
    n=length(objs);
end
...
if(~isempty(xRng))
    for i=1:n
        v=objs(i).bb(1);
        objs(i).ign = objs(i).ign || v<xRng(1) || v>xRng(2);
    end;
end
if(~isempty(xRng))
    for i=1:n
        v=objs(i).bb(1)+objs(i).bb(3);
        objs(i).ign = objs(i).ign || v<xRng(1) || v>xRng(2);
    end;
end
...
if(ignOcc), for i=1:n, if objs(i).occ;  objs(i).ign = ignOcc; end; end; end
...
function [objs, gt0,dt0] = loadAll(gtDir, dtDir, pLoad )
% Load all ground truth and detection bbs in given directories.
%
...
% load ground truth
persistent keyPrv gtPrv objsPrv;
key={gtDir,pLoad};
n=length(gtFs);
...
for i=1:n
    [objs{i},gt0{i}]=bbLoad(gtFs{i},pLoad);
end
...
function [gt,dt,avg_IOU] = evalRes( gt0, dt0, thr, mul,avg_IOU0 )
% Evaluates detections against ground truth data.
%
...
if(nargin<5 || isempty(avg_IOU0)),avg_IOU0 =[]; end
...
if(iscell(gt0) && iscell(dt0) )
    ...
    dt=gt;
    for i=1:n
        [gt{i},dt{i},avg_IOU ] = evalRes(gt0{i},dt0{i},thr,mul,avg_IOU);
    end;
    return;
end
...
%%
gt=[gt0 ord]; % CHANGED: from gt = gt0

gt(:,5)=-gt(:,5);

dt=dt0;
dt=[dt zeros(nd,2)]; %% Changed from zeros(nd,1)
...
for d=1:nd
    ...
    elseif(m==1)
        ...
        dt(d,6)=m;
        dt(d,7)=gt(g,6);
        avg_IOU = [avg_IOU,bstOa];
    end
end

end
function [gt,dt,det_IOU] = evalRes_new( gt0, dt0, thr, mul,det_IOU0)

if(nargin<3 || isempty(thr)), thr=.5; end
if(nargin<4 || isempty(mul)), mul=0; end
if(nargin<6 || isempty(det_IOU0)),det_IOU0 =[]; end
...
if(iscell(gt0) && iscell(dt0))
    ...
    dt=gt;
    for i=1:n
        [gt{i},dt{i},det_IOU] = evalRes_new(gt0{i},dt0{i},thr,mul,det_IOU);
    end;
    return;
end
...
dt=dt0;
dt=[dt zeros(nd,3)]; %% Changed from zeros(nd,1)
...
for d=1:nd
    ...
    for g=1:ng
        % if this gt already matched, continue to next gt
        m = gt(g,5);
        %keep record of the highest iou
        
        if (oa(d,g) >  bstOa_nodet && ~(bstm~=0 && m==-1))
            bstOa_nodet = oa(d,g);
            bstg_nodet = g;
        end
    ...
    elseif(m==1)
        ...
        dt(d,7)=gt(g,6);
        dt(d,8) = bstOa;
        det_IOU = [det_IOU,bstOa];
    elseif(m==0)
        if bstg_nodet > 0
            dt(d,7)=gt(bstg_nodet,6);
        end
        dt(d,8) = bstOa_nodet;
    end
end
...
% compute results
if(size(dt,1)==0)
    xs=0; ys=0; score=0; ref=ref*0; return; end
m=length(ref);
np=size(gt,1);
score=dt(:,5);
tp=dt(:,6);
...
function [xs,ys,score,ref] = compRoc_new( gt, dt, roc, ref, minthr,bgOrloc )
...
if strcmp(bgOrloc,'loc')
    fp=double(tp~=1& iou <= minthr);%
elseif strcmp(bgOrloc,'bg')
    fp=double(tp~=1& iou > minthr);%
end
fp=cumsum(fp);
tp=cumsum(tp);
