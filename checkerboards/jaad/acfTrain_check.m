function detector = acfTrain_check( varargin )
...
% save detector
save([opts.name '_original.mat'],'detector');
...
if( exist(crDir,'dir') && stage==0 )
  ...
  if(positive)
      ...
      i=0;k=0;batch=64;
      load('checkerboards/AcfCaltechseed.mat');
...
if( positive ), bbs=gt; bbs=bbs(bbs(:,5)==0,:); else
  if( stage==0 )
    ...
    bbs=acfDetect_my(I,detector); [~,ord]=sort(bbs(:,5),'descend');
