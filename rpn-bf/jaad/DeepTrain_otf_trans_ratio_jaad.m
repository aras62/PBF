function detector = DeepTrain_otf_trans_ratio_jaad( varargin )
...
%           'gtDir',opts.dataDir,'pLoad',opts.pLoad,...
%           'reapply',1,'show',2, 'nms_thres', opts.nms_thres, ...
%           'conf', opts.conf, 'caffe_net', opts.caffe_net, 'silent', true,...
%           'cache_dir', opts.cache_dir, 'ratio', opts.ratio);
...
%             showboxes2(im, rois(idx).boxes(sel_idx(nms_sel_idxes), :))
    ...
    %                 assert(size(fg_feat_cell{idx}, 2)==opts.feat_len, sprintf('assert fail: feat_len should set to %d', size(fg_feat_cell{idx}, 2)));
                    ...
                    error(sprintf('ERROR: could not read image %s!', opts.imdb_train.image_at(idx)));
                end
            end
        end
