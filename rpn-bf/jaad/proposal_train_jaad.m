function save_model_path = proposal_train_jaad(conf, imdb_train, roidb_train, varargin)
...
%% try to find trained model
    ...
    if exist(save_model_path, 'file')
        fprintf('rpn model already exist at %s\n',save_model_path); 
        return;
    end
...
%% making tran/val data
    ...
    [image_roidb_train, bbox_means, bbox_stds]...
                            = proposal_prepare_image_roidb_jaad(conf, opts.imdb_train, opts.roidb_train, opts.empty_image_sample_step);
    fprintf('Done.\n');
    
    if opts.do_val
        fprintf('Preparing validation data...');
        [image_roidb_val]...
                                = proposal_prepare_image_roidb_jaad(conf, opts.imdb_val, opts.roidb_val, opts.empty_image_sample_step, bbox_means, bbox_stds);
        fprintf('Done.\n');
        % fix validation data
...
function [shuffled_inds, sub_inds] = generate_random_minibatch(shuffled_inds, image_roidb, ims_per_batch, fg_image_ratio)
    ...
    if isempty(shuffled_inds)
        ...
        if ims_per_batch == 1
            ...
            empty_image_inds = arrayfun(@(x) sum(x.bbox_targets{1}(:, 1)==1) == 0, image_roidb, 'UniformOutput', true); % arrayfun
