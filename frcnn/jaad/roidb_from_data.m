function roidb = roidb_from_data(imdb, modelSetup,dataInfo)
...
flip = modelSetup.use_flipped;
anno_path = imdb.annotDir;

pLoad = getTags(dataInfo.dataSet);

pLoad = [pLoad 'hRng',modelSetup.hRng, 'vRng',modelSetup.vRng,...
    'ignOcc', modelSetup.ignOcc, 'squarify', modelSetup.squarify];

if flip
    cache_file = ['./imdb/cache/roidb_' imdb.name '_flip'];
else
...
if modelSetup.regenRoiFile && exist(cache_file, 'file')
    delete(cache_file);
end
...
try
  ...
  files=bbGt_jaad('getFiles',{anno_path});
  num_gts = 0;
  num_gt_no_ignores = 0;
