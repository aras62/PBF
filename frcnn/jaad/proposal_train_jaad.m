...
% inputs
    ...
    ip.addParameter('solver_def_file',     fullfile(pwd, 'proposal_models', 'Zeiler_conv5', 'solver.prototxt'), ...
                                                        @isstr);
    ip.addParameter('net_file',            fullfile(pwd, 'proposal_models', 'Zeiler_conv5', 'Zeiler_conv5.caffemodel'), ...
                                                        @isstr);
    ip.addParameter('cache_name',          'Zeiler_conv5', ...
                                                        @isstr);
    
    ip.parse(conf, imdb_train, roidb_train, varargin{:});
    opts = ip.Results;
    
%    opts.fg_image_ratio = 0.5; rpn_Bf
...
function model_path = snapshot(conf, caffe_solver, bbox_means, bbox_stds, cache_dir, file_name)
    ...
    bbox_pred_layer_name = conf.rpn_bbox_pred;%'proposal_bbox_pred';
