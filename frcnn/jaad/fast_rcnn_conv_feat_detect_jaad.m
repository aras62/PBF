function [pred_boxes, scores] = fast_rcnn_conv_feat_detect_jaad(conf, caffe_net, im, conv_feat_blob, boxes, max_rois_num_in_gpu)
...
% --------------------------------------------------------
    ...
    for i = 1:ceil(total_rois / max_rois_num_in_gpu)
        ...
        if conf.test_binary
            ...
            scores = caffe_net.blobs(conf.fcnn_cls_score).get_data();
