function [input_blobs, random_scale_inds, im_rgb] = proposal_generate_minibatch_jaad(conf, image_roidb)
...
% --------------------------------------------------------
    ...
    %out = floor(val/conf.feat_stride);
    output_size = ceil(img_size./conf.feat_stride);%; [calc_output_size(img_size(1), conf), calc_output_size(img_size(2), conf)];
...
function [labels, label_weights, bbox_targets, bbox_loss_weights] = sample_rois(conf, image_roidb, ...
fg_rois_per_image, rois_per_image, im_scale)

    [anchors, ~] = proposal_locate_anchors_jaad(conf, image_roidb.im_size);
    ...
    if isempty(image_roidb.boxes)
       ...
       [bbox_targets, ~] = ...
           proposal_compute_targets(conf, scale_rois(image_roidb.boxes, image_roidb.im_size, im_scale),...
       gt_ignores, image_roidb.class,  anchors{1}, image_roidb, im_scale);
    end
