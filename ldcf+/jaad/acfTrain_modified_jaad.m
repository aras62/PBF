function detector = acfTrain_modified_jaad( varargin )
...
nm=[opts.name '.mat']; t=exist(nm,'file');
if(t)
    fprintf('Detector named %s already exists. Skip training. \n', nm)
detector=opts; return; end
...
for stage = 0:numel(opts.nWeak)-1
    ...
    if( stage==0 && length(opts.filters)==2 )
        filtsz = opts.filters(1);
        filtnum = opts.filters(2);
...
dfs= { 'pPyramid',{}, 'filters',[], ...
    ...
    'nPerNeg',25, 'nAccNeg',10000, 'pJitter',{}, 'winsSave',0 ,'NITER',2};
...
if(~isempty(opts.pLoad.squarify))
...
if( positive ), bbs=gt; bbs=bbs(bbs(:,5)==0,:); else
    if( stage==0 )
        ...
        bbs=acfDetect_modified(I,detector); [~,ord]=sort(bbs(:,5),'descend');
